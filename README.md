...
<!-- ===== CONTRIBUTION REACTOR CORE (generated by .github/scripts/generate_reactor.py) =====
     Replaces the third-party 3D isometric graph. Same 53-week calendar, but the volume is a
     heat ring (cyan = active week, gold = peak week, graphite = quiet week) around a live core:
     emitters from the plate rim, a dashed counter-rotating ring, a week-boundary dial, and a
     14s sensor sweep that never crosses the number. Peak week, active weeks, average and
     longest daily run are read out around it, plus a measured language mix.
     Header names the instrument; the Stark reference is a caption line. -->
<picture>
